...
- `leon-engineering@leon-local` 是跨宿主框架的唯一规范插件；其 9 个工作流和 7 个职责代理是稳定的共享基线。
...
`leon-engineering` 自己管理的共享核心工作流仍是：`agent-routing`、`bugfix-evidence`、`feature-loop`、`logging-observability`、`project-adapter`、`project-bootstrap`、`project-harness`、`review-ship`、`skill-health`。两类能力按触发条件路由，不能因目录重复而重复安装。
...
判定规则固定为：Claude 的直接目录加上已启用 `leon-engineering` 插件的九个核心工作流；Codex 只计入其直接 `~/.codex/skills` 目录。UI 必须以此脚本的输出为准，不能把“共享目录中的内容等价”误写成“两个宿主各有相同数量的直接安装目录”。
