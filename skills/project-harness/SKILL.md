...
```bash
node scripts/harness-control.mjs --project /absolute/project --transition --task-id implement --status in_progress --reason "执行者开始处理"
node scripts/harness-control.mjs --project /absolute/project --retry --task-id implement --reason "已修正失败原因，明确重试"
